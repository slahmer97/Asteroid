...
void rest_server::init(std::string &host, std::string &port) {
    ...
    m_g_httpHandler->endpoint["^/echo/?$"].on_message = [&](const std::shared_ptr<WsServer::Connection> connection,
    ...
    m_g_httpHandler->endpoint["^/echo/?$"].on_close = [&](const std::shared_ptr<WsServer::Connection> connection,
                                                          int status, const std::string &reason/*reason*/) {

        this->on_close(connection, status, reason);
    };

    // modify handshake response headers here if needed
    m_g_httpHandler->endpoint["^/echo/?$"].on_handshake = [](const std::shared_ptr<WsServer::Connection>&/*connection*/,
                                                             SimpleWeb::CaseInsensitiveMultimap& /*response_header*/) {
        return SimpleWeb::StatusCode::information_switching_protocols; // Upgrade to websocket
    };

    m_g_httpHandler->endpoint["^/echo/?$"].on_error = [](const std::shared_ptr<WsServer::Connection> connection,
...
void rest_server::on_close(std::shared_ptr<WsServer::Connection> connection, int status, const std::string &) {
